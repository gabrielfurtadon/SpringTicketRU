...
public class PedidoController {
	...
	public ResponseEntity<List<Pedido>> findAll() {
		// Pedido u = new Pedido(1L, "Gabriel", "991580417", "gabriel@gmail.com",
		// "gabriel123"); instancção manual mocada, nao precida depois que cria o
		// repository
		List<Pedido> list = controller.findAll();
		return ResponseEntity.ok().body(list); // ok -> retorna resposta com sucesso no HTTP | .body -> corpo da
												// resposta usuario u
	}

	@GetMapping(value = "/{id}") // PARA FALAR QUE A URL TEM UM PARAMETRO que é o id
	public ResponseEntity<Pedido> findById(@PathVariable int id) { // PARA O SPRING ACEITAR O ID COMO PARAMETRO QUE VAI
																	// CHEGAR NA URL
